...
- 管道燃料精炼机：priority 100，225 污水容量；只保留一个专用管道配方，每次消耗 75 污水并生产 35 化学燃料，不搜索实体粪便污泥。
...
管道燃料精炼机明确使用普通 tick，与堆肥桶复用同一套管道接收、30-tick 自动申请和 10-tick 手动转移逻辑。配方菜单只从建筑端注册一次，因此只显示一个专用管道污水配方。
